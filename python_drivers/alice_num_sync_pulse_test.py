# -*- coding: utf-8 -*-
"""
Created on Tue Sep 22 17:00:47 2020
...
import datetime
import james_utils
logfile = "stream_test_results_9_23_num_sync_variation.txt"

def log_to_file(test_num, test_series_num, stream_len, succ, num_errors, num_no_photon, num_bad_range, num_neg_offset, sent_stream, received_stream, nsp):
    
    file = open(logfile,'a')
    
    
    new_line = str(test_num) + ", " + str(test_series_num) + ", " + str(stream_len) + ", success:, " + str(succ) + ", num errors:," + str(num_errors)
    new_line += ", num_no_photon," + str(num_no_photon) + ", num_bad_range," + str(num_bad_range) +  ", num_neg_offset," + str(num_neg_offset) + ", num_sync_pulse," + str(nsp)
    #new_line += ", Sent:, "
...
if(res):
    ...
    #2600
    #16300
    for num_sync_pulse in range(100, 2000, 100):
        
        if(exit_test):
                break
        
        for test_num in range(0, 200):
            ...
            try:
                ...
                log_to_file(count, test_num, stream_len, 0, errs, 0, 0, 0, [], [], num_sync_pulse)
